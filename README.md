...
### Thiết kế nội thất — Bước 1 đến Bước 4 và giao diện trung tâm

Mở `Extensions > SonVu CNC Plugins > Thiết kế nội thất`:

- `Trung tâm nội thất`: bảng điều khiển tiếng Việt tóm tắt tủ, chi tiết ván,
  phụ kiện, cảnh báo và trạng thái giấy phép; từ đây có thể đi thẳng đến từng
  bước của quy trình từ Bước 1 đến Bước 4. Thanh công cụ `SonVu Nội thất` có các nút
  riêng cho Trung tâm, tạo tủ, sửa tủ, danh sách chi tiết, dự toán chi phí và
  tối ưu cắt ván.
...
Hộp tạo/chỉnh sửa tủ được chia thành bốn thẻ: `Thùng tủ` (Bước 1), cùng `Mặt
cánh`, `Ngăn kéo` và `Phụ kiện` thuộc Bước 2. Các nút
`Quay lại`/`Tiếp theo` cho phép đi tuần tự; lệnh chỉnh sửa từ Trung tâm có thể
mở thẳng thẻ liên quan. Các lệnh menu cũ vẫn được giữ lại.
...
Bước 2 hỗ trợ các bố trí mặt trước bằng tiếng Việt:
...
Phần ngăn kéo của Bước 2 tạo hộp năm tấm cho từng mặt ngăn kéo: hai hông,
...
Phần phụ kiện của Bước 2 tạo phụ kiện mẫu: một tay nắm cho mỗi mặt trước, bản lề chén
...
Bước 3 hiển thị hai bảng tiếng Việt riêng cho chi tiết ván và phụ kiện.
...
Bảng danh sách chỉ đọc metadata trong model và không thay đổi hình học. Chức năng xuất bổ
...
Bước 3 cũng có nút `Dự toán chi phí` trong bảng danh sách. Người dùng nhập:
...
công, thuế và xuất CNC chưa thuộc Bước 3.

Bước 4 có công cụ `Tối ưu cắt ván`. Người dùng nhập kích thước tấm
...
Thông số được lưu trong tùy chọn SketchUp sau một lần tính thành công. Bước 4
chỉ đọc danh sách chi tiết và không sửa model.

Bước 4 trực quan hóa từng phương án bằng sơ đồ tấm ván theo đúng tỷ lệ và tọa
độ đã tính. Mỗi nhóm vật liệu/độ dày có nút chuyển giữa các tấm. Trên sơ
...
Bước 4 vẫn chỉ đọc dữ liệu và không tạo hình học trong model.

Bước 4 có nút `Xuất phương án` sau khi tính thành công. Người dùng chọn
...
Việc xuất chỉ xảy ra khi người dùng bấm nút, không ghi dữ liệu vào model. Bước
4 chưa tính trình tự cưa, quản lý ván thừa hoặc tạo đường chạy dao CNC.

Bước 5 bổ sung `Xem trước gia công CNC`. Plugin đọc tủ trong vùng chọn hoặc
toàn bộ model, dựng hệ tọa độ X/Y riêng cho từng chi tiết và chuyển vị trí bản
lề chén thành nguyên công khoan trên Mặt B. Bảng xem trước hiển thị:

- chiều dài, chiều rộng, độ dày và vật liệu của chi tiết;
- sơ đồ vị trí lỗ, tọa độ X/Y, đường kính và chiều sâu;
- trạng thái `Sẵn sàng` hoặc lỗi vượt biên/độ dày;
- tay nắm và ray ngăn kéo dưới dạng tham chiếu chờ mẫu khoan của nhà sản xuất.

Bước 5 chỉ đọc dữ liệu và không khoan/cắt model. Người dùng có thể mở từ Trung
tâm nội thất, menu hoặc thanh công cụ.

Người dùng có thể mở `Quy tắc gia công và mẫu khoan` để chọn mẫu `Tiêu chuẩn
ván 18 mm`, `Chỉ chốt gỗ và hàng lỗ`, `Chỉ bản lề hiện có` hoặc tự nhập thông
số. Bước 5 hỗ trợ thêm:

- lỗ chốt gỗ và ổ cam cho liên kết hông–nóc/đáy;
- hàng lỗ đợt theo bước 32 mm trên hông và hai mặt vách đứng;
- rãnh hậu trên hông, nóc và đáy;
- sơ đồ riêng cho Mặt A và Mặt B;
- cảnh báo khi lỗ hai mặt giao nhau trong chiều dày chi tiết.

Quy tắc hợp lệ được lưu trong tùy chọn SketchUp sau khi người dùng bấm `Cập
nhật xem trước`.

Khi tất cả nguyên công đều hợp lệ, nút `Xuất gói CNC` tạo một thư mục
`<ten_cong_trinh>_cnc` gồm:

- một file DXF đơn vị mm cho mỗi mặt chi tiết có gia công; đường bao nằm trên
  layer `PANEL_OUTLINE`, lỗ và rãnh nằm trên các layer thể hiện loại, kích
  thước và chiều sâu;
- `nguyen_cong.csv`: bảng UTF-8 cho Excel liên kết từng nguyên công với tủ,
  chi tiết, mặt, tọa độ, kích thước, chiều sâu, layer và file DXF.

Nếu còn bất kỳ nguyên công `Cần kiểm tra`, toàn bộ việc xuất bị khóa. Khi ghi
đè, plugin chỉ được thay thế thư mục có dấu nhận dạng do chính SonVu CNC
Plugins tạo. DXF dùng hệ tọa độ của bảng xem trước và không chứa G-code, dao,
tốc độ tiến dao hoặc phép biến đổi riêng của bộ điều khiển; người vận hành vẫn
phải kiểm tra gá đặt và thiết lập trong phần mềm máy.
